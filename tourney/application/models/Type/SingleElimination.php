...
class Model_Type_SingleElimination extends Model_Type_Abstract implements Model_Interface_Tree
{
	...
	protected function _buildTourney()
	{
		...
		if ($this->_dirty) {
			...
			if ($this->_dataObject['gameid'] === NULL) {
				throw new Exception("Game not set for tourney");
			}
	...
	{
		if (isset($this->_dataObject['matchuptype'])) {
			$obj = new $this->_dataObject['matchuptype'];
	...
	/**
	 * (non-PHPdoc)
	 * @see models/Type/Model_Type_Abstract#_typeSpecificGetForm()
	 */
	protected function _typeSpecificGetForm(Zend_Form_SubForm $form)
	{
	...
	{
		$this->_dataObject['matchuptype'] = $data['matchuptype'];
	}
	...
	{
		$this->_dataObject['matchuptype'] = get_class($matchupType);
		$this->_dirty = true;
		return $this;
	}
}